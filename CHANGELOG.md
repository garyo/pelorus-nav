...
### Fixed
- Android: when the system killed the app mid-recording (for example to
  free memory for the camera) and later relaunched it in the background,
  the device GPS stayed off with a "tracking stopped
  (foreground-start-failed)" banner until Retry was tapped. The app now
  restarts the GPS on its own as soon as it is back on screen.
- Stopping a track recording while its last fixes were still being written
  no longer briefly flags a track-save error.
...
- A buoy's fog signal and topmark no longer appear at zooms where the buoy
  itself is still hidden.
...
- When charts or the street basemap fail to load, the app now shows a
  ...
  silently leaving the chart blank.
- The chart readout pill at the bottom of the screen now shows load
  and connectivity status: an amber LOADING CHARTS… note while chart
  detail is still arriving, a red TILE LOAD ERROR or OFFLINE: MISSING
  CHARTS warning while tiles are actually failing (tap it for
  details), or a muted OFFLINE note when there's simply no internet.
...
- Chart regions no longer request tiles for areas outside their own
  coverage, reducing network traffic.
- Deleting a downloaded region could leave it still showing as
  downloaded.
- Stopping a short track recording could leave the new track missing
  from the Tracks panel until something else refreshed it.
