...
- **Overlapping semi-transparent land areas with the street underlay** (mostly fixed): with `streetUnderlay` on, LNDARE is drawn at 30% opacity so streets show through, and the tiles carry overlapping LNDARE polygons wherever ENC cells overlap (same-band compositing merges features without clipping; multi-band compositing keeps a ~1 km buffer for gap prevention). Per-feature `fill-opacity` stacked those into darker patches (e.g. around 42.370,-71.095). Fixed client-side with MapLibre 6's `fill-layer-opacity`, which composites the whole layer once (`applyUnderlay` in `src/chart/osm-underlay.ts`). Remaining case: each region has its own land layer, so where two region *sources* both cover a tile (z0–7 everywhere, all zooms in region-bbox overlap strips such as ny-inland/northern-new-england) the two layers still stack. A single merged tile source for all regions would remove that; the pipeline needs no change.
